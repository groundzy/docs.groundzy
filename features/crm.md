...
**Requests, Quotes, Jobs, Invoices**: **Pro** and **Teams** tiers (Small Team, Mid Team, Large Team, Enterprise). Nav and drawers use `WORKFLOW_TIERS` in `app/lib/drawers.ts`.
...
- **ProOrTeamsAccess**: `useProOrTeamsAccess()` – clients/properties surfaces; upgrade prompt for Home/Plus
- **Workflow pipeline**: capability `workflow.pipeline` via `useCapabilities().has("workflow.pipeline")` — **Pro + all team tiers**. The hook `useTeamsOnlyAccess()` reads this capability (name is historical); used from workflow forms and related views.
- **Upgrade**: `useProCheckout()` for Pro; team create/join flow for Teams
