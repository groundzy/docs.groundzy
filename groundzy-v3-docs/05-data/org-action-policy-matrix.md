...
- **Org CRM workflow stages** (`teams/{teamId}.crmWorkflowProfile.stages`): when a stage is **off** for the org, it is merged into the same effective deny list as presets—each member’s `users.orgPolicy.blockOrgWorkflowReadEntities` is the **union** of preset blocks and org-disabled stages (see app `mergeBlockOrgWorkflowReadEntities`). Server command checks and Stripe Connect invoice payment routes also consult this profile so disabled modules cannot be used end-to-end.
- **`blockOrgCrmModules`** (optional `list` of `client` \| `property`): when present, **org-scoped** Firestore access for **`clients`** and **`properties`** collections is blocked for org insiders (see app `userOrgCrmModuleBlocked` / `orgScopedCrmModuleAccessDenied` in Firestore rules), aligned with `teams/{teamId}.crmWorkflowProfile.coreModules` and tier entitlements (`crm.clients` / `crm.properties`). Server paths that mirror homeowner CRM rows (`ensureShareWithOrgCrm`, `syncDefaultProShare`) skip writes when the target org has disabled the corresponding module.
